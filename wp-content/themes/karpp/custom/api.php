...
add_action('rest_api_init', function () {
    register_rest_route( 'karpp/v1', '/vehicles/(?P<id>\d+)', array(
        'methods' => 'GET',
        'callback' => 'get_vehicle_by_id',
    ));

    register_rest_route( 'karpp/v1', '/vehicles', array(
        'methods' => 'GET',
    ...
    register_rest_route( 'karpp/v1', '/vehicles/filters', array(
        'methods' => 'GET',
 ...
 */
function get_vehicle_by_id(WP_REST_Request $request)
{
 ...
 * @return WP_REST_Response
 */
function get_all_vehicles(WP_REST_Request $request)
{
 ...
 * @return WP_REST_Response
 */
function get_vehicle_filters(WP_REST_Request $request)
{
    $data = [];
    ...
    $car_doors = get_terms(array(
        'taxonomy' => 'car_doors_categories',
        'hide_empty' => false,
    ));

    $data['brands'] = array_map(function($brand){
        return [
            'id' => $brand->term_id,
            'name' => $brand->name,
        ];
    ...
    $data['car_doors'] = array_map(function($car_door){
        return [
            'id' => $car_door->term_id,
            'name' => $car_door->name,
        ];
 ...
 * @return array
 */
function get_vehicle_response($vehicle)
{
    ...
    return [
        ...
        'fipe' => get_field('fipe', $vehicle->ID) ? get_field('fipe', $vehicle->ID) : null,
        'car_doors' => $car_doors->name,
        'axle' => get_field('axle', $vehicle->ID) ? (int) get_field('axle', $vehicle->ID) : 0,
        'add_ons' => $add_ons,
        'whatsapp' => get_field('whatsapp', $vehicle->ID) ? get_field('whatsapp', $vehicle->ID) : null,
